...
import { formatTimeSlot, getTimeRangeForSlot, resolveTimeSlotFromRange } from '@/lib/time-slot';
...
export default function Edit() {
    const { batch, plans } = usePage().props as {
        batch: {
            id: number;
            batch_code: string;
            plan_id: number;
    ...
    const [selectedPlanId, setSelectedPlanId] = useState(String(batch.plan_id));
    const [selectedClassId, setSelectedClassId] = useState(String(batch.class_id));
    const [selectedSubjectId, setSelectedSubjectId] = useState(String(batch.subject_id));
    const [selectedTimeSlot, setSelectedTimeSlot] = useState(batch.time_slot || '');
    const [timeFrom, setTimeFrom] = useState(getTimeRangeForSlot(batch.time_slot)?.from ?? '');
    const [timeTo, setTimeTo] = useState(getTimeRangeForSlot(batch.time_slot)?.to ?? '');
    const [selectedTeacherId, setSelectedTeacherId] = useState(String(batch.teacher_id));
    ...
    useEffect(() => {
        ...
        setSelectedClassId(String(selectedPlan.class_id));

        if (selectedPlan.subjects.length === 1) {
            setSelectedSubjectId(String(selectedPlan.subjects[0].id));
        } else {
            const currentStillValid = selectedPlan.subjects.some((subject) => String(subject.id) === selectedSubjectId);
    ...
    useEffect(() => {
        const fetchEligibleTeachers = async () => {
            ...
            const currentExists = data.some((teacher: { id: number }) => String(teacher.id) === selectedTeacherId);
            if (!currentExists) {
    ...
    }, [selectedSubjectId, selectedTimeSlot, selectedClassId, batch.id]);
    ...
    return (
        <AdminLayout>
            ...
            <div className="flex h-full flex-1 flex-col gap-6 p-6">
                ...
                <Card className="p-6">
                    <Form {...BatchController.update.form({ id: batch.id })} resetOnSuccess disableWhileProcessing defaults={batch}>
                        {({ errors, processing }) => (
                            <div className="mx-auto flex w-full max-w-3xl flex-col gap-6">
                                <section className="space-y-4 rounded-lg border bg-muted/20 p-4">
                                    ...
                                    <div className="grid gap-2">
                                        ...
                                        <Input id="batch_code" name="batch_code" defaultValue={batch.batch_code} required />
                                    ...
                                    <div className="grid gap-2">
                                        ...
                                        <Select
                                            name="plan_id"
                                            defaultValue={String(batch.plan_id)}
                                            onValueChange={(value) => {
                                ...
                                <section className="space-y-4 rounded-lg border bg-muted/20 p-4">
                                    ...
                                    <div className="grid gap-2">
                                        ...
                                        <Select
                                            ...
                                            disabled={!selectedSubjectId || !selectedTimeSlot || teacherOptions.length === 0}
                                        >
                                            <SelectTrigger>
                                    ...
                                    <div className="grid gap-2">
                                        ...
                                        <Input name="students_limit" type="number" min="1" defaultValue={batch.students_limit} required />
