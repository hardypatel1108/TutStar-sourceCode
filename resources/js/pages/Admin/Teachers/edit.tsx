...
const breadcrumbs: BreadcrumbItem[] = [
    ...
    { title: 'Edit Teacher', href: '/admin/teachers' },
];
...
export default function Edit({ teacher, classes }: { teacher: any; classes: ClassItem[] }) {
    const [preview, setPreview] = useState<string | null>(null);

    const defaultLanguages = useMemo(() => {
        if (Array.isArray(teacher?.languages) && teacher.languages.length > 0) return teacher.languages;
        return [''];
    }, [teacher?.languages]);

    const defaultTimings = useMemo(() => {
        const source = teacher?.comfortable_timings;
        if (!source || typeof source !== 'object') return [];

        const rows: Array<{ day: string; from: string; to: string }> = [];
        Object.entries(source).forEach(([day, ranges]) => {
    ...
    }, [teacher?.comfortable_timings]);

    const defaultExperiences = useMemo(() => {
        const list = teacher?.teaching_experiences ?? teacher?.teachingExperiences;
        if (Array.isArray(list) && list.length > 0) {
            return list.map((exp: any) => ({
                class_id: String(exp.class_id ?? ''),
                subject_id: String(exp.subject_id ?? ''),
                experience_years: exp.experience_years !== null && exp.experience_years !== undefined ? String(exp.experience_years) : '',
                description: exp.description ?? '',
            }));
        }
        return [{ class_id: '', subject_id: '', experience_years: '', description: '' }];
    }, [teacher?.teaching_experiences, teacher?.teachingExperiences]);
    ...
    useEffect(() => {
        experiences.forEach((exp, index) => {
            if (exp.class_id) {
                fetchSubjectsForExperienceRow(index, exp.class_id);
            }
        });
    }, []);
    ...
    return (
        <AdminLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="flex h-full flex-1 flex-col gap-6 p-6">
                ...
                <Card className="p-6">
                    <Form {...TeacherController.update.form({ teacher: teacher.id })} disableWhileProcessing className="mx-auto flex w-full max-w-5xl flex-col gap-6">
                        {({ processing, errors }) => (
                            <>
                                ...
                                {Object.keys(errors).length > 0 && (
                                    <div className="rounded-lg border border-red-200 bg-red-50 p-4 text-sm text-red-700">
                                        <p className="mb-2 font-semibold">Please fix the following errors before saving:</p>
                                ...
                                <div className="grid gap-6 lg:grid-cols-2">
                                    <section className="space-y-4 rounded-lg border bg-muted/20 p-4">
                                        ...
                                        <div className="flex flex-col items-center gap-3 pb-2">
                                            <label className="group cursor-pointer">
                                                ...
                                                {preview || teacher.user.profile_image ? (
                                                    <img
                                                        src={preview ? preview : `/storage/${teacher.user.profile_image}`}
                                                        alt="Profile"
                                                    ...
                                                    <Avatar className="h-28 w-28 overflow-hidden rounded-full border">
                                                        <AvatarFallback className="text-lg font-semibold">{teacher.user.name?.charAt(0)}</AvatarFallback>
                                                    </Avatar>
                                                )}
                                            </label>
                                            <p className="text-xs text-muted-foreground">Click to change profile image</p>
                                    ...
                                    <section className="space-y-4 rounded-lg border bg-muted/20 p-4">
                                        ...
                                        <div className="grid grid-cols-2 gap-4">
                                            <div className="grid gap-2">
                                                <Label>City</Label>
                                                <Input name="city" defaultValue={teacher.city ?? ''} aria-invalid={!!errors.city} className={errors.city ? 'border-destructive' : ''} />
                                                <InputError message={errors.city} />
                                            </div>
                                            <div className="grid gap-2">
                                                <Label>State</Label>
                                                <Input name="state" defaultValue={teacher.state ?? ''} aria-invalid={!!errors.state} className={errors.state ? 'border-destructive' : ''} />
                                        ...
                                        <div className="grid gap-2">
                                            ...
                                            <Input
                                                ...
                                                name="contact_email"
                                                type="email"
                                                defaultValue={teacher.contact_email ?? ''}
                                        ...
                                        <div className="grid gap-2">
                                            ...
                                            <Input
                                                id="contact_mobile"
                                                name="contact_mobile"
                                                defaultValue={teacher.contact_mobile ?? ''}
                                                aria-invalid={!!errors.contact_mobile}
                                                className={errors.contact_mobile ? 'border-destructive' : ''}
                                            />
