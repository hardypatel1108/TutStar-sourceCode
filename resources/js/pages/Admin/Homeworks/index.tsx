...
import { index } from '@/routes/admin/homeworks';
...
export default function Index() {
    const { homeworks, filters, teachers, batches, subjects } = usePage().props as {
        homeworks: any;
        filters: any;
            ...
            <div className="flex flex-col gap-6 p-6">
                ...
                <div className="rounded-lg border bg-card p-3">
                    <div className="mb-3">
                        ...
                        <p className="text-sm text-muted-foreground">Refine homeworks</p>
                    </div>
                    ...
                    <div className="flex flex-wrap items-end gap-2">
                        <div className="flex flex-col gap-1">
                            ...
                            <Input
                                title="Search by homework title..." placeholder="Search by homework title..."
                                value={search}
                                onChange={(e) => {
                ...
                <Card className="p-4">
                    <Table>
                        ...
                        <TableBody>
                            {homeworks.data.length > 0 ? (
                                homeworks.data.map((hw: HomeworkItem, idx: number) => (
                                    <TableRow key={hw.id}>
                                        <TableCell>{(homeworks.current_page - 1) * homeworks.per_page + (idx + 1)}</TableCell>
                                        <TableCell className="font-medium">{hw.title || '-'}</TableCell>
                                        <TableCell>{hw.session?.batch?.name || '-'}</TableCell>
                                        <TableCell>{hw.session?.batch?.clazz?.name || '-'}</TableCell>
                                        <TableCell>{hw.session?.subject?.name || '-'}</TableCell>
                                        <TableCell>{hw.session?.teacher?.user?.name || '-'}</TableCell>
                                        <TableCell>{hw.due_date_formatted || '-'}</TableCell>
                                        <TableCell>{hw.posted_time_formatted || '-'}</TableCell>
                                    </TableRow>
                                ))
                            ) : (
                                <TableRow>
                    ...
                    {homeworks.total > homeworks.per_page && (
